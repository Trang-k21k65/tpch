...
void gen_tbl(int tnum, DSS_HUGE start, DSS_HUGE count, long upd_num) {
    ...
    for (i = start; count; count--, i++) {
        ...
        case SUPP:
            mk_supp(i, &supp);
            if (set_seeds == 0) tdefs[tnum].loader(&supp, upd_num);
            break;
        case CUST:
            mk_cust(i, &cust);
            if (set_seeds == 0) tdefs[tnum].loader(&cust, upd_num);
            break;
        case PSUPP:
        case PART:
        case PART_PSUPP:
            mk_part(i, &part);
            if (set_seeds == 0) tdefs[tnum].loader(&part, upd_num);
            break;
        case NATION:
            mk_nation(i, &code);
            if (set_seeds == 0) tdefs[tnum].loader(&code, 0);
            break;
        case REGION:
            mk_region(i, &code);
            if (set_seeds == 0) tdefs[tnum].loader(&code, 0);
...
void usage(void) {
    fprintf(stderr, "%s\n%s\n\t%s\n%s %s\n\n", "USAGE:", "dbgen [-{vf}][-T {pcsoPSOL}]",
            "[-s <scale>][-C <procs>][-S <step>]", "dbgen [-v] [-O m] [-s <scale>]",
            "[-U <updates>]");
...
void process_options(int count, char** vector) {
    ...
    while ((option = getopt(count, vector, "b:C:d:fi:hO:P:qs:S:T:U:v")) != -1) switch (option) {
...
void validate_options(void) {
    ...
    if (children != 1) {
        ...
        if (step == -1) {
            fprintf(stderr,
                    "ERROR: -S must be specified when generating data in multiple chunks\n");
            exit(-1);
        }
    }
    ...
    if (step != -1) {
        if ((children == 1) && (updates == 0)) {
            fprintf(stderr,
                    "ERROR: -S is only valid when generating data in multiple chunks or generating "
                    "updates\n");
            exit(-1);
        }
    }
...
int main(int ac, char** av) {
    ...
    table = (1 << CUST) | (1 << SUPP) | (1 << NATION) | (1 << REGION) | (1 << PART_PSUPP) |
            (1 << ORDER_LINE);
    force = 0;
    insert_segments = 0;
    delete_segments = 0;
    ...
    load_dists();
#ifdef RNG_TEST
    for (i = 0; i <= MAX_STREAM; i++) Seed[i].nCalls = 0;
#endif
        ...
        if (table & (1 << i)) {
            if (children > 1 && i < NATION) {
                ...
                if (i < NATION)
                    ...
                    rowcnt = tdefs[i].base;
                if (verbose > 0) fprintf(stderr, "Generating data for %s", tdefs[i].comment);
