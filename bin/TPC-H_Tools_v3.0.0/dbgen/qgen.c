...
void qsub(char* qtag, int flags) {
    ...
    if (flags & DFLT_NUM) fprintf(ofp, SET_ROWCOUNT, rowcnt);
    while (fgets(line, BUFSIZ, qfp) != NULL) {
        if (!(flags & COMMENT)) strip_comments(line);
        mark = line;
        while ((cptr = strchr(mark, VTAG)) != NULL) {
            *cptr = '\0';
            ...
            case 'b':
            case 'B':
                if (!(flags & ANSI)) fprintf(ofp, "%s\n", START_TRAN);
                cptr++;
                break;
            case 'c':
            case 'C':
                if (flags & DBASE) fprintf(ofp, SET_DBASE, db_name);
                cptr++;
                break;
            case 'e':
            case 'E':
            ...
            case 'O':
                if (flags & OUTPUT)
                    fprintf(ofp, "%s '%s/%s.%d'", SET_OUTPUT, osuff, qtag, (snum < 0) ? 0 : snum);
                cptr++;
                break;
            case 'q':
            case 'Q':
            ...
            case 'X':
            case 'x':
                if (flags & EXPLAIN) fprintf(ofp, "%s\n", GEN_QUERY_PLAN);
                cptr++;
                break;
            case '1':
            case '2':
...
int process_options(int cnt, char** args) {
    ...
    while ((flag = getopt(cnt, args, "ab:cdhi:n:Nl:o:p:r:s:t:vx")) != -1) switch (flag) {
        ...
        case 's': /* scale of data set to run against */
            ...
            if (scale > MAX_SCALE)
                fprintf(stderr, "%s %5.0f %s\n%s\n", "WARNING: Support for scale factors >",
                        MAX_SCALE, "GB is still in development.",
...
int main(int ac, char** av) {
    ...
    if (flags & VERBOSE)
        fprintf(ofp, "-- TPC %s Parameter Substitution (Version %d.%d.%d build %d)\n", NAME,
                VERSION, RELEASE, PATCH, BUILD);

    setup();

    if (!(flags & DFLT)) /* perturb the RNG */
    {
        if (!(flags & SEED)) rndm = (long)((unsigned)time(NULL));
