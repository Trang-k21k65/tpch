...
long mk_order(DSS_HUGE index, order_t* o, long upd_num) {
    ...
    if (asc_date == NULL) asc_date = mk_ascdate();
    ...
    if (ocnt > 0) o->orderstatus = 'P';
    if (ocnt == o->lines) o->orderstatus = 'F';
...
long mk_time(DSS_HUGE index, dss_time_t* t) {
    ...
    while (d > months[m].dcnt + LEAP_ADJ(y, m)) m++;
    PR_DATE(t->alpha, (int)y, (int)m, (int)(d - months[m - 1].dcnt - ((LEAP(y) && m > 2) ? 1 : 0)));
    t->year = 1900 + y;
