    ...
    if (updates) {
        if (update > 0) /* updates */
            if (insert_segments) {
                ...
                sprintf(upath, "%s%c%s.u%d.%d", env_config(PATH_TAG, PATH_DFLT), PATH_SEP,
            ...
            if (delete_segments) {
                ++delete_segment;
                sprintf(upath, "%s%cdelete.u%d.%d", env_config(PATH_TAG, PATH_DFLT), PATH_SEP,
                        -update % 10000, delete_segment);
            } else {
                sprintf(upath, "%s%cdelete.%d", env_config(PATH_TAG, PATH_DFLT), PATH_SEP, -update);
            }
...
int pr_cust(customer_t* c, int mode) {
    ...
    if (fp == NULL) fp = print_prep(CUST, 0);
...
int pr_part(part_t* part, int mode) {
    ...
    if (p_fp == NULL) p_fp = print_prep(PART, 0);
...
int pr_psupp(part_t* part, int mode) {
    ...
    if (ps_fp == NULL) ps_fp = print_prep(PSUPP, mode);
...
int pr_supp(supplier_t* supp, int mode) {
    ...
    if (fp == NULL) fp = print_prep(SUPP, mode);
...
int pr_nation(code_t* c, int mode) {
    ...
    if (fp == NULL) fp = print_prep(NATION, mode);
...
int pr_region(code_t* c, int mode) {
    ...
    if (fp == NULL) fp = print_prep(REGION, mode);
...
int pr_drange(int tbl, DSS_HUGE min, DSS_HUGE cnt, long num) {
    ...
    for (child = min; cnt > 0; child++, cnt--) {
        ...
        if (delete_segments) {
            if (rows_per_segment == 0) rows_per_segment = (cnt / delete_segments) + 1;
