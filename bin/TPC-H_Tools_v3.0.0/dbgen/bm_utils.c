...
void a_rnd(int min, int max, int column, char* dest) {
    ...
    for (i = 0; i < len; i++) {
        if (i % 5 == 0) RANDOM(char_int, 0, MAX_LONG, column);
...
long unjulian(long date) {
    ...
    for (i = STARTDATE / 1000; i < date / 1000; i++) res += 365 + LEAP(i);
    res += date % 1000 - 1;
...
void read_dist(char* path, char* name, distribution* target) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (sscanf(line, "%[^|]|%ld", token, &weight) != 2) continue;
...
FILE* tbl_open(int tbl, char* mode) {
    ...
    if (retcode) {
        ...
        if (S_ISREG(fstats.st_mode) && !force && *mode != 'r') {
            ...
            if (!yes_no(prompt)) exit(0);
            f = fopen(fullpath, mode);
        } else if (S_ISFIFO(fstats.st_mode)) {
            retcode = open(fullpath, ((*mode == 'r') ? O_RDONLY : O_WRONLY) | O_CREAT, 0664);
...
set_state(int table, long sf, long procs, long step, DSS_HUGE* extra_rows) {
    ...
    *extra_rows = rowcount % procs;
    ...
    for (i = 0; i < step - 1; i++) {
        ...
        if (tdefs[table].child != NONE) tdefs[tdefs[table].child].gen_seed(0, rowcount);
    }
