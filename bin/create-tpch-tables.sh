...
if [[ ${SCALE_FACTOR} -ne 1 ]] && [[ ${SCALE_FACTOR} -ne 10 ]] && [[ ${SCALE_FACTOR} -ne 30 ]] && [[ ${SCALE_FACTOR} -ne 100 ]] && [[ ${SCALE_FACTOR} -ne 1000 ]] && [[ ${SCALE_FACTOR} -ne 10000 ]]; then
...
if [[ ${SCALE_FACTOR} -eq 1 ]]; then
    ...
    echo "Run SQLs from ${CURDIR}/../ddl/create-tpch-tables-sf10.sql"
    mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" <"${CURDIR}"/../ddl/create-tpch-tables-sf10.sql
elif [[ ${SCALE_FACTOR} -eq 30 ]]; then
    echo "Run SQLs from ${CURDIR}/../ddl/create-tpch-tables-sf30.sql"
    mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" <"${CURDIR}"/../ddl/create-tpch-tables-sf30.sql
